...
public:
	...
	void pop(size_t ind) 
	{
		if (ind >= sz) 
		{
			throw std::out_of_range("index bigger the size!");
		}
		sz--;
		Node* cur = head;
	...
	T& at(size_t ind)
	{
		if (ind >= sz)
		{
			throw std::out_of_range("index bigger then size!");
		}
	...
	void insert(size_t ind, const T& elem)
	{
		if (ind > sz)
		{
		...
		Node* newNode = new Node{ elem, cur->next };
		cur->next = newNode;
		if (cur == tail)
		{
			tail = newNode;
